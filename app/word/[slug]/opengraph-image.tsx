  ...
  if (!w) {
    return new ImageResponse(
      <div style={{ display: 'flex', width: '100%', height: '100%', backgroundColor: '#0d9488', color: 'white', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
        <div style={{ display: 'flex', fontSize: 48 }}>VOCABWIZE</div>
      </div>,
      { ...size }
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', backgroundColor: '#f0fdfa', fontFamily: 'sans-serif', padding: '48px 56px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', fontSize: 18, color: '#0d9488', fontWeight: 700, letterSpacing: 2 }}>VOCABWIZE</div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginTop: 8 }}>
            <div style={{ display: 'flex', fontSize: 64, fontWeight: 900, color: '#0c1a2e', lineHeight: 1 }}>
              {w.word}
            </div>
            {w.phonetic && (
              <div style={{ display: 'flex', fontSize: 24, color: '#94a3b8', fontStyle: 'italic' }}>/{w.phonetic}/</div>
            )}
          </div>
          {w.pos && (
            <div style={{ display: 'flex', fontSize: 20, color: '#0d9488', marginTop: 6, fontStyle: 'italic' }}>{w.pos}</div>
          )}
        </div>
        {levelStyle && w.level && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', backgroundColor: levelStyle.bg, borderRadius: 16, padding: '16px 28px' }}>
            <div style={{ display: 'flex', fontSize: 13, color: levelStyle.text, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>Level</div>
            <div style={{ display: 'flex', fontSize: 28, fontWeight: 800, color: levelStyle.text, marginTop: 4 }}>
      ...
      <div style={{ display: 'flex', backgroundColor: 'white', borderRadius: 12, padding: '24px 28px', border: '2px solid #99f6e4', marginTop: 8 }}>
        <div style={{ display: 'flex', fontSize: 22, color: '#1e293b', lineHeight: 1.5 }}>
          {displayDef}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: '#94a3b8' }}>
        <div style={{ display: 'flex', color: '#0d9488', fontWeight: 600 }}>vocabwize.com</div>
